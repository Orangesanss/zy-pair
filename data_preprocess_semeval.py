...
def xml2txt(file_path):
    ...
    '''
    output = file_path.replace('.xml', '_text_singer.txt')
    sent_list = []
    ...
    with open(origin_file, 'rb') as fopen:
        ...
        for sentence in root:
            ...
            for c in terms:
                ...
                print('target: ',target)
                print('aspect: ',example['tokens'][left_word_offset:to_word_offset])
            ...
            idx+=1
    extended_filename = origin_file.replace('.xml', '_biaffine_depparsed_singer.json')
...
def main():
    ...
    for test_file in data:
        ...
        #     os.path.join(args.data_path, train_file.replace('.xml', '_text_singer.txt')), predictor)
        test_sentences = get_dependencies(os.path.join(
            args.data_path, test_file.replace('.xml', '_text_singer.txt')), predictor)
