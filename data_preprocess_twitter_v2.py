...
def main():
    ...
    train_sentences_with_dep = get_dependencies(os.path.join(args.data_path, 'train_v2.txt'), predictor)
    test_sentences_with_dep = get_dependencies(os.path.join(args.data_path, 'test.txt'), predictor)
    print(len(train_sentences), len(test_sentences))

    # to json
    syntaxInfo2json(train_sentences, train_sentences_with_dep, os.path.join(args.data_path, 'train_v2.txt'))
    syntaxInfo2json(test_sentences, test_sentences_with_dep, os.path.join(args.data_path, 'test_v2.txt'))
