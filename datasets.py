...
def get_dataset(dataset_name):
    ...
    laptop_train_pair = '../data/semeval14_pair/Laptop_Train_v2_biaffine_depparsed_pair.json'
    laptop_test_pair = '../data/semeval14_pair/Laptops_Test_Gold_biaffine_depparsed_pair.json'

    laptop_train_singer = '../data/semeval14_pair/Laptop_Train_v2_biaffine_depparsed_singer.json'
    laptop_test_singer = '../data/semeval14_pair/Laptops_Test_Gold_biaffine_depparsed_singer.json'

    twitter_train = '../data/twitter/train_biaffine.json'
    twitter_test = '../data/twitter/test_biaffine.json'

    ds_train = {'rest': rest_train,
                'laptop_pair': laptop_train_pair, 'laptop_singer': laptop_train_singer, 'twitter': twitter_train}
    ds_test = {'rest': rest_test,
               'laptop_pair': laptop_test_pair, 'laptop_singer': laptop_test_singer, 'twitter': twitter_test}
...
def reshape_dependency_tree_new(as_start, as_end, dependencies, multi_hop=False, add_non_connect=False, tokens=None,
    ...
    for i in range(as_start, as_end):
        for dep in dependencies:
            ...
            if i == dep[1] - 1:
                # not root, not aspect
                # (dep[2] - 1 < as_start or dep[2] - 1 >= as_end) 非方面词
...
def get_rolled_and_unrolled_data(input_data, args):
    ...
    for e in input_data:
        ...
        for i in range(len(e['aspect_sentiment'])):
            ...
            frm = e['from_to'][i][0]  # 方面词位置开始
            to = e['from_to'][i][1]  # 方面词位置结束
...
def load_and_cache_vocabs(data, args):
    ...
    ''' 4.4 建立词表日志文件，包含：数据集的名字、embeding的类型、词表的长度、日志文件路径+名'''
    if args.embedding_type == 'glove':
        ...
        ''' 4.5 建立embeding嵌入文件，包含：embeding的文件夹、日志文件路径+名'''
...
def load_glove_embedding(word_list, glove_dir, uniform_scale, dimension_size):
    ...
    for word in word_list:
        if word in word2offset:
            ...
            word_vectors.append(
                np.random.uniform(-uniform_scale, uniform_scale, dimension_size))  # 如果glove词表文件中没有该词，则随机赋值
    return word_vectors
...
def build_text_vocab(data, vocab_size=100000, min_freq=2):
    ...
    words_and_frequencies = sorted(counter.items(), key=lambda tup: tup[0])  # 统计词频数
...
def my_collate(batch):
    ...
    sentence_ids = pad_sequence(
        sentence_ids, batch_first=True, padding_value=0)  # 只接受等长输入，将序列转化为经过填充以后的一个长度相同的新序列新序列。
    ...
    _, sorted_idx = text_len.sort(descending=True)  # 将一个batch中的text_len排序，并返回值的索引位置
    sentence_ids = sentence_ids[sorted_idx]  # 根据索引位置的
