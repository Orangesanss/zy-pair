...
def train(args, train_dataset, model, test_dataset,word_vocab):
    ...
    collate_fn = get_collate_fn(args)  # 将 batch 中的句子按照长度进行排序
    ...
    if args.embedding_type == 'bert':
        ...
        parameters = filter(lambda param: param.requires_grad, model.parameters())  # 从参数param集合-- parameters中找到可变的param形成一个新的集合。
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            torch.nn.utils.clip_grad_norm_(
                model.parameters(), args.max_grad_norm) # 梯度裁剪
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    ''' 输出每个model的预测结果，包括样本和预测'''
                    if i%5==0:
                        eval_case = evaluate_case(args, test_dataset, model, word_vocab)
                        eval_path = os.path.join(args.output_dir,'model_epoch_%s_pair'%i)
                        if not os.path.exists(eval_path):
                            os.makedirs(eval_path)
                        eval_file = os.path.join(eval_path, 'case_'+args.eval_file)
                        with open(eval_file, 'a+') as writer:
                            for i in range(len(eval_case)):
                                writer.write("sentence:  %s\n  aspect: %s\n pred: %s label: %s\n" \
                                             % (eval_case[i]['sentence'],eval_case[i]['aspect'],eval_case[i]['pred'],eval_case[i]['label']))
                                writer.write('\n')
                            writer.write('\n')
...
def evaluate(args, eval_dataset, model):
    ...
    output_eval_file = os.path.join(args.output_dir, args.eval_file)
...
def evaluate_case(args, eval_dataset, model, word_vocab):
    ...
    # for batch in tqdm(eval_dataloader, desc='Evaluating'):
        ...
        if args.embedding_type == 'bert':
            sent_ids = inputs['input_ids'][0].detach().cpu().numpy()
            # aspect_ids = inputs['input_aspect_ids'][0].detach().cpu().numpy()
            aspect_ids = batch[2][0].detach().cpu().numpy()
            case = {}
            case['sentence'] = args.tokenizer.decode(sent_ids)
            case['aspect'] = args.tokenizer.decode(aspect_ids)
            case['pred'] = pred
