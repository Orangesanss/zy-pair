...
# from model_gcn import (Rel_GAT, GCN, GAT)
...
def parse_args():
    ...
    # Required parameters
    parser.add_argument('--dataset_name', type=str, default='laptop_singer',
                        choices=['rest', 'laptop_pair', 'laptop_singer', 'twitter'],
                        help='Choose absa dataset.')
    parser.add_argument('--output_dir', type=str, default='../output/zy-pair',
                        help='Directory to store intermedia data, such as vocab, embeddings, tags_vocab.')
    parser.add_argument('--log_file', type=str, default='singer-purebert_droput0.3_20220804.log',
                        help='Directory to store intermedia data, such as vocab, embeddings, tags_vocab.')
    parser.add_argument('--eval_file', type=str, default='pair_eval_results_purebert_droput0.3_20220804.txt',
                        help='Directory to store intermedia data, such as vocab, embeddings, tags_vocab.')
    ...
    parser.add_argument('--bert_model_dir', type=str, default='../data/bert-base-uncased',
                        help='Path to pre-trained Bert model.')
    parser.add_argument('--pure_bert', action='store_true',
                        help='Cat text and aspect, [cls] to predict.')
    # parser.add_argument('--pure_bert', default=True,
    #                   help='Cat text and aspect, [cls] to predict.')
    parser.add_argument('--gat_bert', action='store_true',
                        help='Cat text and aspect, [cls] to predict.')

    parser.add_argument('--highway', action='store_true',
                        help='Use highway embed.')  # action='store_true',，表示有参数传入则为True
    # parser.add_argument('--highway', default=True,
    #                     help='Use highway embed.') #action='store_true',，表示有参数传入则为True
    ...
    parser.add_argument('--gat_our', action='store_true',
                        help='GAT_our')
    # parser.add_argument('--gat_our', default=True,
    #                    help='GAT_our')
...
def main():
    ''' 1 设置日志文件，记录训练过程中的事件'''
    args = parse_args()
    log_paths = os.path.join(args.output_dir, 'logs')
    if not os.path.exists(log_paths):
        os.makedirs(log_paths)
    # Setup logging
    logging.basicConfig(
        filename=os.path.join(log_paths, args.log_file),
    ...
    if args.embedding_type == 'bert':
        tokenizer = BertTokenizer.from_pretrained(args.bert_model_dir)  # 获取预训练模型
    ...
    # model = Rel_GAT(args, dep_tag_vocab['len'], pos_tag_vocab['len'])  # R-GAT with reshaped tree

    model.to(args.device)
    # Train
    _, _, all_eval_results = train(args, train_dataset, model, test_dataset,word_vocab)
